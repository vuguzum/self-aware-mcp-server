...
server.tool("get_current_location", "Returns the current location passed as a command line argument when starting the MCP server.", {}, async () => {
    return {
        content: [
            {
                type: "text",
...
server.tool("get_current_date_time", "Get current date and time as weekday, year-month-day hours:minutes:seconds", {}, async () => {
    const now = new Date();
...
server.tool("get_current_system", "Returns the main operating system parameters including name, version, displayName, platform, and architecture.", {}, async () => {
    const systemInfo = {
        os: {
            name: os.type(),
            version: os.release(),
...
server.tool("calculate", `Эта функция позволяет вычислять математические выражения, переданные в виде строки,
